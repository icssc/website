...
import { BIT_AND_BYTES_EVENTS, type BitsAndBytesEvent } from "./bits-and-bytes-event-data";
...
function TimelineList({ events }: TimelineListProps) {
    return (
        <div className="relative w-full">
            <ul
                className="grid [&>li]:grid-cols-[0_min-content_1fr] w-full [mask-image:linear-gradient(to_bottom,transparent_0%,black_3rem,black_calc(100%-3rem),transparent_100%)]"
            >
                {events.map((event) => (
...
function TimelineItem({ event }: TimelineItemProps) {
    return (
        ...
        >
            ...
            <div className="col-start-2 col-end-3 row-start-1 row-end-2 mx-auto w-0.5 bg-ic-pink -z-10" style={{ height: 'calc(100% + 3rem)' }} />
...
function EventMeta({ timeIso, location }: EventMetaProps) {
    return (
        <div className="flex flex-col items-start space-y-4 md:col-span-1 text-left">
            <p className="text-nowrap text-xl font-bold">{formatEventDate(timeIso)}</p>
...
function EventContent({ title, description }: EventContentProps) {
    return (
        <div className="col-span-1 flex flex-col items-start space-y-3 md:col-span-2">
            ...
            {description && (
                <p className="text-pretty text-left text-gray-700 line-clamp-6">{description}</p>
            )}
        </div>
    );
}
